...
1. List all `.md` files in `spec-bot/standards/` and its subfolders
...
**Note:** `root` is a reserved keyword — it refers to `.md` files directly in `spec-bot/standards/` (not in a subfolder). Do not create an actual folder named "root".

### Step 2: Load Existing Index

Read `spec-bot/standards/index.yml` if it exists. Note which entries already have descriptions.
...
Generate `spec-bot/standards/index.yml` with this structure:
...
**Note:** `root` appears first and contains standards files that live directly in `spec-bot/standards/` (not in subfolders).
...
Updates `spec-bot/standards/index.yml`
