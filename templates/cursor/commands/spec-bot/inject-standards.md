...
**Note:** `root` is a reserved keyword — it refers to `.md` files directly in `spec-bot/standards/` (not in a subfolder).
...
Read `spec-bot/standards/index.yml` to get the list of available standards and their descriptions.
...
@spec-bot/standards/api/response-format.md
@spec-bot/standards/api/error-handling.md
@spec-bot/standards/global/naming.md
...
Arguments can be:
- **Folder name** — `api` → inject all `.md` files in `spec-bot/standards/api/`
- **Folder/file** — `api/response-format` → inject `spec-bot/standards/api/response-format.md`
- **Root folder** — `root` → inject all `.md` files directly in `spec-bot/standards/` (not in subfolders)
- **Root file** — `root/naming` → inject `spec-bot/standards/naming.md`
